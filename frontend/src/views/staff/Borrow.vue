  ...
  methods: {
    async loadBorrows() {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:8000/borrows', { params })
    ...
    async handleDelete(row) {
      try {
        await ElMessageBox.confirm(
          `貸出記録 ${row.id} を削除しますか？この操作は元に戻せません。`,
          '削除確認',
          {
        ...
        const response = await axios.delete(`http://localhost:8000/borrows/${row.id}`)
